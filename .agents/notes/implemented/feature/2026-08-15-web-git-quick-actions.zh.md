...
- **`@deepseek-ai/dsh-host-workspace-git`（`ctx.workspaceGit`）** — web 宿主服务，通过 `ctx.subprocess` 在工作区目录运行 git，复用面向模型工具包的纯解析器（`parseStatus`/`parseFileDiff`/`parseLog`/`parseBranches`）、错误词汇（`GitError`）、固定调用与 `GIT_TERMINAL_PROMPT=0` 固定，使 git 输出解析只有一个归属。方法：`status`、`diff`（`git diff HEAD --no-ext-diff -- <file>` 解析成 context/added/deleted 记录的 hunk，供查看器变更标记使用；未跟踪文件或无提交仓库返回 `kind: 'untracked'` 且 hunk 为空）、`commit`（先全部暂存再提交 — 面板的快速提交语义，不同于工具的仅暂存提交）、`push`、`pull`、`branches`、`checkout`。每个操作融合调用者信号与自有截止时间，解析命令要求完整 stdout（有损读取即 `GIT_OUTPUT_OVERFLOW`），并拒绝相对 wire 路径。
...
- **客户端接线** — 运行时 `IWorkspaces` face 新增 `gitStatus`/`gitDiff`/`gitCommit`/`gitPush`/`gitPull`/`gitBranches`/`gitCheckout`（抛出类型化的 `GitOperationError`），fixture 与测试双打实现新方法。
- **`@deepseek-ai/dsh-client-ui-workspace-git`** — 面板填充由 ui-workspace-files 的 FileTree 条目声明并渲染在树下方的 `sidebar.files.git` 子插槽。它显示分支及 ahead/behind 事实、变更文件桶（已暂存／未暂存／未跟踪／冲突）、全部暂存提交框、推送／拉取／刷新操作与分支切换器，非仓库显示「不是 git 仓库」提示。变更文件行的文件名是按钮：把 git 报告路径（工作区相对或绝对）按会话 cwd 解析后，通过树的 owner 打开手势在共享查看器中打开。面板状态存放在共享 store；操作独占（忙碌标志），失败的错误行在操作后的状态重载中保留。
...
- web bundle 增加两个包与七个 RPC 方法；模块图、配置目录、客户端插槽目录与翻译对随之重新生成。由于面板直接组合在文件树上，同一变更也修复了既有的 file-browser/workspace-files 注册漂移（缺失的 tsconfig 路径映射与 aggregate 引用）。
- 查看器中的逐行 git 变更标记——`git.diff` RPC 的消费方——是独立能力；见 [web git change marks and open-from-git rows](2026-08-16-web-git-change-marks.md)。
