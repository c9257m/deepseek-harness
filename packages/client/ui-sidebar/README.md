...
Sidebar shell plugin: the wordmark, New Session action, layout-owned collapse control, the sessions/files browse-view switch, the scroll-aware region seat, and the bottom-pinned Settings seat. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; [ui-workspace-files](../ui-workspace-files/README.md) owns the workspace file tree rendered into `sidebar.files`; this package neither derives their rows nor owns their view preferences. The two-tab browse view is shell-local presentation state: the region area renders the `sidebar.workspaces` or `sidebar.files` occupant by the selected tab, and each occupant keeps its own state across the switch. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces`, `sidebar.files`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
