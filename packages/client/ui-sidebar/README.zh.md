...
侧边栏外壳插件：负责字标、New Session 操作、布局持有的折叠控件、会话/文件浏览视图切换、可感知滚动的区域 seat，以及固定在底部的 Settings seat。[ui-workspace](../ui-workspace/README.md) 持有渲染到 `sidebar.workspaces` 的 Workspace 与 Session 浏览器；[ui-workspace-files](../ui-workspace-files/README.md) 持有渲染到 `sidebar.files` 的工作区文件树；本包既不派生其中的行，也不持有其视图偏好。双页签浏览视图属于外壳局部呈现状态：区域按所选页签渲染 `sidebar.workspaces` 或 `sidebar.files` 的占用者，切换时各自保留自身状态。折叠到布局拥有的 56px 轨道仍属于本地呈现行为。约定：[slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md)。
...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.workspaces`、`sidebar.files` 与 `sidebar.settings` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。
