...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the five child declarations', async () => {
    ...
    expect(slots.spec('workspace.fileViewer')).toEqual({ kind: 'single', scope: 'root' })
  ...
  it('injects no business face and attaches the layout actions', async () => {
    ...
    await fiber.await()
    const actions = {
      setSidebar: vi.fn(), setDetails: vi.fn(), toggleSidebar: vi.fn(), openDetails: vi.fn(),
      closeDetails: vi.fn(), enterFileMode: vi.fn(), exitFileMode: vi.fn(),
    }
    ...
    layout.openFileMode()
    expect(actions.enterFileMode).toHaveBeenCalledOnce()
    layout.closeFileMode()
    expect(actions.exitFileMode).toHaveBeenCalledOnce()
  })
