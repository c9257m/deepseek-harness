...
describe('LayoutController', () => {
  it('forwards the five panel actions to the attached set', () => {
    ...
    service.openFileMode()
    service.closeFileMode()

    expect(panels.toggleSidebar).toHaveBeenCalledTimes(1)
    expect(panels.openDetails).toHaveBeenCalledTimes(1)
    expect(panels.closeDetails).toHaveBeenCalledTimes(1)
    expect(panels.enterFileMode).toHaveBeenCalledTimes(1)
    expect(panels.exitFileMode).toHaveBeenCalledTimes(1)
  ...
  it('fails loud before the root entry wired its actions', () => {
    ...
    expect(() => { service.openFileMode() }).toThrow(/panel actions not wired/)
    expect(() => { service.closeFileMode() }).toThrow(/panel actions not wired/)
  })
