...
- **一对文档是三个同目录文件。** 英文 `foo.md`、中文 `foo.zh.md`，加一份一致性记录 `foo.i18n.yaml`，都在同一目录。不用语言目录，不用独立翻译仓库，不用中英混排的单文件。配对必须整体合并：PR（Pull Request）永远不会只带一种语言而缺其余两个文件。配对可以反向：此时不带后缀的 `foo.md` 是简体中文侧（默认展示的文档），英文侧使用 `foo.en.md` 后缀。反向配对通过 [scripts/translation-pairing-record.ts](../../scripts/translation-pairing-record.ts) 中的 `REVERSED_PAIR_STEMS` 按 stem 声明；根 README 是第一个反向配对（`README.md` 为中文，`README.en.md` 为英文）。其余配对规则一律不变。
...
- **语言切换行。** 中文侧一律在 H1 标题后立即以 `[English](<英文侧>) | 中文` 链回英文；普通撰写的英文源在同一位置以 `English | [中文](<中文侧>)` 互链；清单内的生成英文源省略此行，以便与生成器输出逐字节一致。普通配对的两侧是 `foo.md`（英文）与 `foo.zh.md`（中文）；反向配对的两侧是 `foo.en.md` 与 `foo.md`。发布到 GitHub 以外位置的 README（例如 PyPI 项目元数据）可以改用指向同一对侧文件的规范 `https://github.com/deepseek-ai/deepseek-harness/blob/master/<repository-path>` URL，使切换行在该位置仍可访问。
